...
def eval_split(loader, model, split, opt):
    ...
    while True:
        ...
        for i, sent_id in enumerate(sent_ids):
            ...
            # #根据求sub有关的obj的和
            # scores=scores.view(pool5.size(1),pool5.size(1))
            # scores=torch.sum(scores,dim=1)
            # sort_scores=torch.sort(torch.nn.functional.softmax(scores),descending=True)
            # scores_var=torch.var(sort_scores[0])

            #所有score过softmax，并求var
            sort_scores=torch.sort(torch.nn.functional.softmax(scores),descending=True)
            scores_var=torch.var(sort_scores[0])

            pred_ix = torch.argmax(scores)

            # # 删除最大score类别的score，便于比较最大类别和第二大类别score的差距
            # list_scores=list(scores)
            # label_num=pred_ix.item()//pool5.size(1)
            # if (label_num+1)*pool5.size(1)<len(list_scores):
            #     del_list_scores=list_scores[0:label_num*pool5.size(1)]+list_scores[(label_num+1)*pool5.size(1):]
            # else:
            #     del_list_scores=list_scores[0:label_num*pool5.size(1)]
            # sort_scores=torch.sort(scores,descending=True)
            # del_list_scores.sort(reverse=True)
            # sort_del_scores=del_list_scores

            pred_ann_id = expand_ann_ids[pred_ix]
            # pred_ann_id = expand_ann_ids[pred_ix*pool5.size(1)]
            ...
            if opt['use_IoU'] > 0:
                if IoU >= 0.5:
                    acc += 1
                    corr_max_scores.append(sort_scores[0][0].item())
                    corr_var.append(scores_var.item())
                    # corr_diff_scores.append((sort_scores[0][0]-sort_scores[0][1]).item())
                    # corr_diff_scores.append((sort_scores[0][0]-sort_del_scores[0]).item())
                else:
                    error_max_scores.append(sort_scores[0][0].item())
                    error_var.append(scores_var.item())
                    # error_diff_scores.append((sort_scores[0][0]-sort_scores[0][1]).item())
                    # error_diff_scores.append((sort_scores[0][0]-sort_del_scores[0]).item())
            else:
                if pred_ix == gd_ix:
                    acc += 1
    ...
    score_result={'corr_max':corr_max_scores,'corr_diff':corr_diff_scores,'error_max':error_max_scores,'error_diff':error_diff_scores,'corr_var':corr_var,'error_var':error_var}

    return loss_sum / loss_evals, acc / loss_evals, predictions, score_result
