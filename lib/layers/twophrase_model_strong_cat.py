...
class PairEncoder(nn.Module):
    ...
    def forward(self, ann_fc7, ann_fleats):
        ...
        ann_fc7 = self.fc7_normalizer(ann_fc7.contiguous())                                         #[ann_num,1024]
        ...
        return pair_feats, expand_1_fc7,  expand_0_fc7                    #[ann_num * ann_num, (1024+512)*2]
...
class TwoPhrase(nn.Module):
    def __init__(self, opt):
        ...
        self.linear_catvl = nn.Linear(self.fc7_dim+300, 1024)

        self.linear_Kt = nn.Linear(1024,512)
        self.linear_Kd = nn.Linear(1024*3,1536)
        self.linear_Kr = nn.Linear(1024,512)
        self.linear_Qt = nn.Linear(300,512)
        self.linear_Qd = nn.Linear(300*3,1536)
        self.linear_Qr = nn.Linear(300,512)
    ...
    def vis_label_fuse(self,labelids,ann_fc7):
        ...
        for i in range(batch_num):
            ...
            img_ann_emb=torch.cat([img_ann_fc7,img_label_emb],1)           #[ann_num,2048+300]
            img_ann_emb=self.linear_catvl(img_ann_emb)                     #[ann_num,1024]
    ...
    def forward(self, sub_wordids, sub_classids, obj_wordids, rel_wordids,labelids, ann_fc7, ann_fleats, psent_to_pann):
        ...
        final_sub,final_rel,final_obj=0.4,1,0.4
        ...
        for i in range(batch_num):
            ...
            K_expand_1_fc7=self.linear_Kt(expand_1_fc7)                      #[ann_num * ann_num, 512]
            Q_sub_fuseemb=self.linear_Qt(img_sub_fuseemb)                   #[sent_num, 512]
            sub_att=self.attention(Q_sub_fuseemb,K_expand_1_fc7)            #[sent_num,ann_num * ann_num]

            K_pair_feats=self.linear_Kd(pair_feats)                          #[ann_num * ann_num, 1536]
            Q_pair_wordemb=self.linear_Qd(img_pair_wordemb)                   #[sent_num, 1536]
            pair_att=self.attention(Q_pair_wordemb,K_pair_feats)

            K_expand_0_fc7=self.linear_Kr(expand_0_fc7)                      #[ann_num * ann_num, 512]
            Q_obj_wordemb=self.linear_Qr(img_obj_wordemb)                    #[sent_num, 512]
