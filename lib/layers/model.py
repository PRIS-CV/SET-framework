...
class KPRN(nn.Module):
    ...
    def forward(self, pool5, fc7, lfeats, dif_lfeats, cxt_fc7, cxt_lfeats, dist, labels, enc_labels, dec_labels, att_labels, select_ixs, att_weights,
        ...
        pair_attn = self.pair_attn(pair_wordembs, pair_feats)   #pair_wordembs:[sent_num,word_emb] pair_feats:[sent_num,ann_num*ann_num,vis_emb] pair_attn:[sent_num,ann_num*ann_num]
        ...
        re_pair_feats = torch.matmul(pair_attn_gumbel.view(sent_num, 1, ann_num*ann_num), pair_feats)
        re_pair_feats = re_pair_feats.reshape([sent_num, -1])  #[sent_num,vis_emb]
        ...
        re_sub_feats = torch.matmul(sub_attn_gumbel.view(sent_num, 1, ann_num*ann_num), expand_1_fc7)
        ...
        re_obj_feats = torch.matmul(obj_attn_gumbel.view(sent_num, 1, ann_num * ann_num), expand_0_fc7)
        ...
        # final_attn = 2*sub_attn_gumbel + 1*obj_attn_gumbel + 1*pair_attn_gumbel

        #根据求sub有关的obj的和,过sofmax，算信息熵
        sum_final_attn=final_attn.view(sent_num,ann_num,ann_num)
        sum_final_attn=torch.sum(sum_final_attn,dim=2)
        sum_final_attn=torch.nn.functional.softmax(sum_final_attn,dim=1)
        D=torch.Tensor([2]*sent_num*ann_num).view(sent_num,ann_num).float().cuda()
        Entropy=torch.log(sum_final_attn)/torch.log(D)
        Entropy=sum_final_attn*Entropy
        loss_entr=torch.sum(Entropy)/sent_num
        loss_sum=loss_sum+0.0005*loss_entr
