...
def evaluate(params):
    ...
    model_prefix = osp.join('twophrase_output', params['dataset_splitBy'], params['id'], 'mrcn_cmr_with_st')
    ...
    # val_loss, acc, predictions, score_result = eval_utils.eval_split(loader, model, split, model_opt)
    ...
    out_dir = osp.join('twophrase_results', params['dataset_splitBy'], 'easy')
    if not osp.isdir(out_dir):
        os.makedirs(out_dir)
    out_file = osp.join(out_dir, params['id'] + '_' + params['split'] + '.json')
    with open(out_file, 'w') as of:
        # json.dump({'predictions': predictions, 'acc': acc, 'score_result':score_result}, of)
