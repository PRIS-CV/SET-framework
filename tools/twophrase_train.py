...
from layers.twophrase_model import TwoPhrase
import evals.utils as model_utils
import evals.twophrase_eval as eval_utils
...
def main(args):
    ...
    while True:
        ...
        tic = time.time()
        data=loader.getBatch_weak('train', opt)
        sub_wordids, sub_classids, obj_wordids, rel_wordids,labelids, ann_fc7, ann_fleats=\
            data['sub_wordids'],data['sub_classwordids'],data['obj_wordids'],data['rel_wordids'],data['label'],data['ann_fc7'],data['ann_fleats']
        ...
        print('i[%s], e[%s],loss=%.3f, sub_loss=%.3f, obj_loss=%.3f, rel_loss=%.3f, contra_loss=%.3f, lr=%.2E, time=%.3f h' % (iter, epoch, loss.data[0].item(), sub_loss.data[0].item(), obj_loss.data[0].item(), rel_loss.data[0].item(), contra_loss.data[0].item(), lr, total_time))
